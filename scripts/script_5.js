...
function menu() {
  let string =
    "[1] Est-ce que tous les livres ont été au moins empruntés une fois<br>";
...
function moreBorrowed(array) {
  let result = array.reduce((acc, val) => {
    return acc && acc.rented > val.rented ? acc : val;
  });
...
function perform() {
  ...
  switch (USER_INPUT.value) {
    case "1":
      OUTPUT_2.innerHTML =
        "<strong>Est-ce que tous les livres ont été au moins empruntés une fois:</strong><br><br>" +
        borrowedOnce(books);
      break;
    case "2":
      OUTPUT_2.innerHTML =
        "<strong>Quel est livre le plus emprunté ?</strong><br><br>" +
        moreBorrowed(books);
      break;
    case "3":
      OUTPUT_2.innerHTML =
        "<strong>Quel est le livre le moins emprunté ?</strong><br>" +
        lessBorrowed(books);
      break;
    case "4":
      id = 873495;
    ...
    case "5":
      ...
      OUTPUT_2.innerHTML =
        "<strong>Supprime le livre avec l'ID: 133712</strong> (" +
        findBook(books, id) +
        ")<br>" +
        print(array);
      break;
    case "6":
      id = 133712;
      OUTPUT_2.innerHTML =
        "<strong>Trie les livres par ordre alphabétique (sans ID: 133712)</strong><br><br>" +
        print(sortBooks(array));
      break;
    default:
      break;
  }
